...
| **Demo-resilience engineering** — a self-contained demo tab so a live-tenant hiccup can't kill a recording: the cockpit renders the agents' captured reconciliation output (`src/lib/reconDemo.ts`) behind a manual demo/live toggle, no network required. | `src/lib/reconDemo.ts`, `src/App.tsx`, `src/components/ReconciliationView.tsx` | `npm run build`; open the app — the Reconciliation tab renders offline |
