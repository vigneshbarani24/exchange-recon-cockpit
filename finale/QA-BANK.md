...
> Because the Actions service isn't provisioned on this tenant — the probe returns, verbatim, "Service: actions not found in Organization". So the reviewer surface is the Maestro instance itself, and the decision is a correlated message where the message *name* is the decision — which is deterministic and auditable: no free text to parse, no ambiguity about what was decided. In production this becomes an Action Center task or a Teams approval with the approver's identity attached to the decision. And I'm volunteering that boundary deliberately — knowing exactly where the tenant ends and the product begins is part of the governance story.
...
> Three things, in order: land the held write-back once the upstream MCP key bug is fixed — it's coded and one server-side fix away. Second, harden the escalation path — and the enriched twin of this flow is already deployed with exactly that: a ten-minute timer SLA on the human gate, Gmail and Slack notification legs each shielded by boundary error events so a connector failure can never block governance, and the policy task — smoke results: {{SMOKE-APPROVE}} / {{SMOKE-ESCALATE}} / {{SMOKE-TIMER}}. Moving the approval itself into Teams, where the reviewers already work, is the roadmap leg on top of that. Third, make quality continuous with tool-trajectory assertions and CI-gated regression in Test Cloud, on top of the twelve passing evals. I know exactly what's real and what's next — that's the point.
