...
> Yes — there's a Test Manager project, "Exchange Recon - Agent Evals", holding all twelve documented eval cases with their results, grouped in a test set named 12/12 passing, so a QA team lands exactly where they'd expect to look. Even that had a story: the create route wasn't where the API's sibling paths suggested, and we found it through the service's own swagger — filed as a discoverability finding. Wiring the set into Test Cloud automation is the roadmap step that makes it continuous.
